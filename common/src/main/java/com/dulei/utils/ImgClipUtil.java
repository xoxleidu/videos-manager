...
public class ImgClipUtil {
    ...
    protected static Logger logger = LoggerFactory.getLogger(ImgClipUtil.class);
    ...
    public static boolean yPic(String first, String second, String out, int border) {
        ...
        try {
            ...
            /* 1 读取第一张图片begin */
            int t_height = y_height - heightTwo; // 图片是横图，逆时针旋转90度再等比缩放
            if (width > height) {
                imageFirst = rotateImageLeft90(imageFirst);
            }
            // 等比缩放
            imageFirst = resize(imageFirst, y_width, t_height); // 缩放后图片的大小
            ...
            int[] imageArrayFirst = new int[(width - border) * height];
            // 从图片中读取RGB
            ...
            g.dispose(); // 留边框
            // 设置左半部分的RGB
            imageResult.setRGB(border, 0, (width - border * 2), height, imageArrayFirst, 0, (width - border));
            // 设置右半部分的RGB
            imageResult.setRGB(0, height, widthTwo, heightTwo, imageArraySecond, 0, widthTwo);
    ...
    public static boolean maigaoPic(String source, String out, int border) {
        ...
        try {
            ...
            imageFirst = resize(imageFirst, y_width, y_height); // 缩放后图片的大小
            width = imageFirst.getWidth();// 图片宽度
            height = imageFirst.getHeight();// 图片高度
    ...
    public static BufferedImage resize(BufferedImage source, int targetW, int targetH) {
        ...
        return zoomInImage(source, targetW, targetH); // 图片宽高都太小时，强制放大图片
      /*
    ...
    public static void discernImg(String img) {
        try {
            ...
            BufferedImage bi = ImageIO.read(fileOne); // 获取图像的宽度和高度
            int width = bi.getWidth();
            int height = bi.getHeight(); // 扫描图片
